...
class TcpCommunicator:
        ...
        def _createMyCustomizedSocket(self):
            ...
            if "win" in sys.platform:
                print("Socket Keep options are Window's")
                self._socket.ioctl(socket.SIO_KEEPALIVE_VALS, (1, self._keepaliveduration, 2))
            else:
                print("Socket Keep options are Linux's")
        ...
        def _accept(self):
                ...
                if self._videoStreamingEnable:
                    import VideoStream
                    self._videoStream=VideoStream.VideoStream(addr[0], "5000")
        ...
        def _parse(self, data):
            ...
            del tokens[len(tokens)-1] # delete last element as it will be empty due to the split() which will split the last ; into an empty token
